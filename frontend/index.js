...
function PionSession(signalerUri, sessionParams, rtcPeerConfiguration) { // eslint-disable-line no-unused-vars
	...
	const startExchange = (ws, peerConnection, remoteSessionKey) => {
		...
		peerConnection
			.createOffer({voiceActivityDetection: true, offerToReceiveVideo: true, offerToReceiveAudio: true})
			.then(createdOffer => {
				offer = createdOffer
			...
			.then(() => {
				ws.send(JSON.stringify({method: 'sdp', args: {dst: remoteSessionKey, sdp: offer}}))
			})
