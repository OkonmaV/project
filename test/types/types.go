...
const (
	...
	OperationCodeGiveMeOutsideAddr    OperationCode = 8
	OperationCodeSetOutsideAddr       OperationCode = 9
)

func FormatAddress(netw NetProtocol, addr string) []byte { // with len specified
	addr_byte := []byte(addr)
	formatted := make([]byte, 3, 3+len(addr_byte))
	binary.BigEndian.PutUint16(formatted, uint16(len(addr_byte)))
	formatted[2] = byte(netw)
	return append(formatted, addr_byte...)
}

func UnformatAddress(raw []byte) (NetProtocol, string, error) { // no len specified
	if len(raw) < 1 {
		return 0, "", errors.New("weird data")
	}
