...
type config struct {
	...
	MgoAddr        string
	MgoColl        string
	MgoCollDeleted string
	mgoSession     *mgo.Session
}

const thisServiceName httpservice.ServiceName = "folders.deletefolder"
...
func (c *config) CreateHandler(ctx context.Context, connectors map[httpservice.ServiceName]*httpservice.InnerService) (httpservice.HttpService, error) {
	...
	return NewHandler(col, c.mgoSession.DB(c.MgoDB).C(c.MgoCollDeleted), connectors[authGetServiceName], connectors[tokenDecoderServiceName])
}
